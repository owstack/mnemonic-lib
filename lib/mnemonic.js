...
var Mnemonic = function(data, wordlist) {
  ...
  if (Buffer.isBuffer(data)) {
    ...
    throw new owsCommon.errors.InvalidArgument('data', 'Must be a Buffer, a string or an integer');
  }
  ent = ent || 128;
  ...
  if (ent % 32 !== 0 || ent < 128) {
    throw new owsCommon.errors.InvalidArgument('ENT', 'Values must be ENT > 128 and ENT % 32 == 0');
  }
...
Mnemonic.fromSeed = function(seed, wordlist) {
  ...
  $.checkArgument(lodash.isArray(wordlist) || lodash.isString(wordlist), 'wordlist must be a string or an array.');
 ...
 * Optionally receive a passphrase network.
 *
 * @param {String=} [passphrase]
 * @param {Network|String|number=} [network] - A supported network; e.g., 'BTC', 'LTC'
 * @returns {HDPrivateKey}
 */
Mnemonic.prototype.toHDPrivateKey = function(passphrase, network) {
  var seed = this.toSeed(passphrase);
  return keyLib.HDPrivateKey.fromSeed(seed, network);
};
